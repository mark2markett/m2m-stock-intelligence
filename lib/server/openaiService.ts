import 'server-only';
import type { OptionsData, M2MScorecard, OptimalTrade, OptionContract } from '@/lib/types';
...
`;
    ...
    const prompt = `You are an institutional-grade trade structuring analyst. Based on the complete technical analysis data below, determine the OPTIMAL TRADE for ${data.symbol}.
...
3. STRATEGY SELECTION — choose the SINGLE best strategy from:
...
4. CONTRACT SELECTION: When recommending options, reference SPECIFIC contracts from the available data above — use actual strikes, expirations, bid/ask prices. Choose contracts with:
   - Delta between 0.25-0.40 for directional plays
   - 14-45 days to expiration for swing trades
   - Adequate liquidity (volume + OI)

5. RISK PARAMETERS: Use ATR for stop placement, support/resistance for targets.` : `TASK: Setup quality is LOW — no high-confidence trade exists. Instead of recommending a trade, identify the SPECIFIC conditions that would need to develop for a quality setup (bullish OR bearish) to appear. Be precise with indicator levels and price action.`}
...
Return a JSON object with EXACTLY these fields:
{
  ...
  "strategy": "name of the strategy (e.g., 'Long Stock', 'Bull Call Spread', 'Bear Put Credit Spread', 'Long Call', 'Iron Condor')",
  "reasoning": "2-3 sentences explaining WHY this is the optimal structure given the setup, IV environment, and available contracts",
  "entryZone": "specific price or price range for entry",
  "stopLoss": "specific level with reasoning",
  "target1": "conservative target with level",
  "target2": "extended target with level",
  "riskRewardRatio": "X.X:1 calculated from entry/stop/target1",
  ...
  "timeHorizon": "expected holding period based on setup stage and expiration",
  "conditionsNeeded": ${!hasSetup ? '["condition 1 that must develop", "condition 2", "condition 3", etc.]' : 'null or omit'},
...
${!hasSetup ? 'For the LOW quality scenario: set vehicle to "stock", direction to "neutral", strategy to "No Trade - Monitoring", and populate conditionsNeeded with 3-6 SPECIFIC conditions (e.g., "RSI needs to break above 50 from current 42.3", "MACD histogram must turn positive", "Price needs to close above EMA20 at $185.50"). Leave legs as an empty array. Entry/stop/target fields should describe the levels that WOULD apply if conditions develop.' : ''}

CRITICAL: Only reference actual contract data provided above. Do not invent strikes or prices. Use educational language throughout — "the data suggests", "historical patterns indicate", never "you should" or "we recommend".`;
    ...
    const response = await fetch(this.API_URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are an institutional-grade trade structuring analyst for the M2M Stock Intelligence platform. Your role is to determine the single optimal trade structure based on technical analysis, options flow, and IV environment. Frame all output as educational observation — never as personal investment advice. Return ONLY valid JSON.'
          },
          {
            role: 'user',
            content: prompt
