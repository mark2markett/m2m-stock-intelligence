...
export class SupportResistanceAnalyzer {
  ...
  private static calculatePivotStrength(highs: number[], lows: number[], index: number, lookback: number): number {
    // Calculate strength based on recent touches (last 30 bars) to avoid overweighting old levels
    let strength = 1;
    const pivotPrice = highs[index] || lows[index];
    const tolerance = pivotPrice * 0.015; // 1.5% tolerance

    // Look at recent 30 bars or all available bars if fewer
    ...
    // Give bonus strength for recent pivots (recency bias)
    const recency = highs.length - index;
    if (recency <= 10) {
      strength *= 1.5; // Recent pivots are more relevant
    } else if (recency <= 20) {
      strength *= 1.2;
    }
  ...
  private static clusterLevels(levels: number[], tolerance: number = 0.015): number[] {
    ...
    for (let i = 1; i < sorted.length; i++) {
      ...
      if (diff <= tolerance) {
        ...
        // Finalize current cluster (use weighted average based on strength if available)
  ...
  // Get most relevant support and resistance levels
  static getKeyLevels(pivots: PivotPoint[], currentPrice: number): { support: number[]; resistance: number[] } {
    ...
    // Cluster nearby levels to avoid too many close levels
    ...
    // Sort by proximity to current price and take top 3
    const supportLevels = clusteredSupport
      .sort((a, b) => Math.abs(b - currentPrice) - Math.abs(a - currentPrice)) // Closest support first
      .slice(0, 3);

    const resistanceLevels = clusteredResistance
      .sort((a, b) => Math.abs(a - currentPrice) - Math.abs(b - currentPrice)) // Closest resistance first
      .slice(0, 3);
