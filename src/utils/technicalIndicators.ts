...
export class TechnicalIndicators {
  ...
  static adx(highs: number[], lows: number[], closes: number[], period: number = 14): number {
    ...
    // Calculate DX values for each period
  ...
  static detectVolatilityRegime(
    ...
    // Calculate historical ATR percentages for comparison
  ...
  static computeIndicators(
    ...
    // Adjust periods based on volatility regime
    let fastSpan: number, slowSpan: number, rsiPeriod: number;
    
    // Use standard periods - don't over-optimize
    fastSpan = baseFast;
    slowSpan = baseSlow;
    rsiPeriod = baseRsi;
    ...
    const macd = this.macd(closes, 12, 26, 9); // Standard MACD periods
    ...
    console.log(`Indicators for ${timeframe} timeframe. Volatility regime: ${regime.toUpperCase()}, EMA20: ${fastSpan}, EMA50: ${slowSpan}, RSI: ${rsiPeriod}`);
  ...
  // RSI interpretation for PDF (matches Python logic exactly)
  static interpretRsiForPdf(
    rsi: number,
    price: number,
    fastEma: number,
    slowEma: number,
    ...
    // Trend regime: EMA + MACD (be careful with extreme ADX values)
    const uptrend = fastEma > slowEma && macdLine > 0;
    const downtrend = fastEma < slowEma && macdLine < 0;
    const strongTrend = adx !== null && adx > 25 && adx < 60; // Flag extreme ADX values

    if (rsi > 85) {
      return {
        zone: "Exhaustion",
        scoreAdjustment: -20,
        rationale: `RSI extremely overbought (>85): exhaustion risk—potential mean reversion. ${adx && adx > 60 ? 'Extreme ADX suggests trend exhaustion.' : ''}`,
        trendRegime: "Overextended"
      };
    } else if (rsi > 65) {
      if (uptrend && strongTrend) {
        return {
          zone: "Strong Bull",
          scoreAdjustment: 15,
          rationale: `RSI in strong bull zone (>65), price above EMAs, MACD > 0${strongTrend ? ', moderate ADX confirms trend' : ''}—trend continuation favored.`,
          trendRegime: "Bull"
        };
      }
      return {
        zone: "Bull",
        scoreAdjustment: 5,
        rationale: "RSI strong (>65), trend positive but not all confirmations present.",
        trendRegime: "Bull"
      };
    } else if (rsi > 55) {
      return {
        zone: "Bull",
        scoreAdjustment: 5,
        rationale: "RSI above 55, bullish momentum favored.",
        trendRegime: "Bull"
      };
    } else if (rsi < 15) {
      return {
        zone: "Exhaustion",
        scoreAdjustment: -20,
        rationale: `RSI extremely oversold (<15): exhaustion risk—potential mean reversion. ${adx && adx > 60 ? 'Extreme ADX suggests trend exhaustion.' : ''}`,
        trendRegime: "Overextended"
      };
    } else if (rsi < 35) {
      if (downtrend && strongTrend) {
        return {
          zone: "Strong Bear",
          scoreAdjustment: 15,
          rationale: `RSI in strong bear zone (<35), price below EMAs, MACD < 0${strongTrend ? ', moderate ADX confirms trend' : ''}—downtrend confirmed.`,
          trendRegime: "Bear"
        };
      }
      return {
        zone: "Bear",
        scoreAdjustment: 5,
        rationale: "RSI below 35, bearish momentum favored.",
        trendRegime: "Bear"
      };
    } else if (rsi < 45) {
      return {
        zone: "Bear",
        scoreAdjustment: 5,
        rationale: "RSI under 45, bears in control.",
        trendRegime: "Bear"
      };
      ...
      return {
        zone: "Neutral",
        scoreAdjustment: -10,
        rationale: "RSI in neutral zone (45–55), lacking clear trend—lower quality setup.",
        trendRegime: "Chop"
      };
    } else {
      return {
        zone: "Neutral",
        scoreAdjustment: 0,
        rationale: "RSI in transition—no clear directional edge.",
        trendRegime: "Chop"
      };
    }
  }
